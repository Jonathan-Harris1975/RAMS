...
class TestParsePlan:
    ...
    def test_empty_changes_are_safe_noop(self):
        assert _parse_plan(json.dumps(_patch([])), "t")["changes"] == []
    ...
    def test_raises_on_unknown_operation(self):
        with pytest.raises(PatchPlanError):
            _parse_plan(json.dumps(_patch([{"file": "x", "operation": "create"}])), "t")

    def test_raises_on_replace_missing_find(self):
        with pytest.raises(PatchPlanError):
            _parse_plan(
                json.dumps(
                    _patch([{"file": "x", "operation": "replace", "replace": "x"}])
                ),
                "t",
            )
...
class TestPlan:
    def test_prompt_requires_direct_anchor_patch_contract(self):
        assert (
            "AnchorPatch/v1" in SYSTEM_PROMPT
            and "Do not return taskId or operations" in SYSTEM_PROMPT
            and "markdown fences" in SYSTEM_PROMPT
        )
    ...
    def test_read_side_sibling_prefix_traversal_blocked(
        self, settings, mock_router, tmp_path
    ):
        repo = tmp_path / "repo"
        repo.mkdir()
