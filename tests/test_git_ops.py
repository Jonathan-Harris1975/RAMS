...
class TestStageAndCommit:
    ...
    def test_commits_and_returns_sha(self) -> None:
        ...
        result = stage_and_commit(
            repo, ["index.html"], "fix: add canonical", dry_run=False
        )
...
class TestPushBranch:
    ...
    def test_skipped_when_push_not_enabled(self) -> None:
        repo = _make_mock_repo()
        push_branch(
            repo, "rms-qa/on-brand/2026-05-05", dry_run=False, push_enabled=False
        )
        repo.git.push.assert_not_called()
    ...
    def test_pushes_when_enabled(self) -> None:
        repo = _make_mock_repo()
        push_branch(
            repo, "rms-qa/on-brand/2026-05-05", dry_run=False, push_enabled=True
        )
...
def test_ensure_clean_branch_blocks_when_active_branch_is_main(tmp_path):
    ...
    subprocess.run(
        ["git", "init", "-b", "main"], cwd=tmp_path, check=True, stdout=subprocess.PIPE
    )
    subprocess.run(
    ...
    with pytest.raises(git_ops.BranchSafetyError):
        git_ops.ensure_clean_branch(tmp_path, "rms-qa/test")
