...
RAMS keeps four reviewable source/lock pairs: runtime (`requirements.in` / `requirements-runtime.lock`), packaging bootstrap (`requirements-bootstrap.in` / `requirements-bootstrap.txt`), PEP 517 build tooling (`requirements-build.in` / `requirements-build.txt`) and CI/development (`requirements-dev.in` / `requirements-dev.txt`). Every lock is generated by pip-tools, pins exact versions and carries SHA-256 hashes for the available distribution artefacts. `pyproject.toml` reads runtime dependency metadata dynamically from `requirements.in`. The generated runtime lock deliberately uses a non-manifest filename so repository architecture scanners do not misclassify the lock as a second, competing Python dependency manifest.
...
```bash
python -m piptools compile --generate-hashes --allow-unsafe --no-emit-index-url --no-emit-trusted-host --no-strip-extras --output-file=requirements-runtime.lock requirements.in
