...
Production direct dependencies are exact-version pinned in `requirements.in`, which is the single Dependabot-visible direct dependency source. `requirements-runtime.lock` is the compiled transitive production set consumed by CI and Docker, while `pyproject.toml` loads its runtime dependency metadata dynamically from `requirements.in`. Dependency upgrades must regenerate `requirements-runtime.lock` and pass the RAMS test and security gates before merge. Do not reintroduce a root `requirements.txt` or a second hard-coded runtime dependency list; the non-standard lock filename is intentional to keep generated lock data distinct from dependency manifests.
