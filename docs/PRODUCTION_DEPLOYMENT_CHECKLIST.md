...
RMS_AIMS_REPO_URL
GITHUB_TOKEN_WEBSITE_AUDITS or RMS_GITHUB_TOKEN
RMS_API_KEY
...
`RMS_PUSH_ENABLED=false` and `RMS_CREATE_PR=false` are intentional. They prevent automatic Git publication while still allowing governed production workflow execution and validated report artefacts.
...
- `/health` and `/livez`: `status=ok` and all three pipelines listed as `idle` or `running`.
...
- `/ops/excellence`: `status=healthy` when the audits bucket verifies, otherwise `status=degraded`; includes `liveWriteControls`, `deploymentContract`, `modelProviderPolicy` and `auditStorage`.
