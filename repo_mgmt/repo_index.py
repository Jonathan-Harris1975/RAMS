...
_EXPRESS_RE = re.compile(
    r"(?:app|router|[A-Za-z0-9_]+Router)\s*\.\s*(?:get|post|put|patch|delete|use|all)\s*\(\s*['\"]([^'\"]+)['\"]"
)
...
def _load_gitignore(repo_root: Path) -> Any:
    ...
    if _HAS_PATHSPEC:
        return _pathspec_mod.PathSpec.from_lines("gitwildmatch", pats)
    return [
        p.strip().rstrip("/")
        for p in pats
...
def _discover_routes(repo_root: Path, file_list: list[str]) -> list[str]:
    routes = {f for f in file_list if f.startswith("pages/") or f.startswith("app/")}
    candidates = []
    for f in file_list:
        p = Path(f)
        name = p.name.lower()
        if f in {"server.js", "server.ts"}:
            candidates.append(f)
        elif f.startswith("routes/") and p.suffix in {".js", ".mjs", ".cjs", ".ts"}:
            candidates.append(f)
        elif f.startswith("audits/routes/") and p.suffix in {
            ".js",
            ".mjs",
            ".cjs",
            ".ts",
...
def build_node_index(repo_root: Path) -> dict[str, Any]:
    ...
    for f in files:
        by.setdefault(Path(f).suffix or "(none)", []).append(f)
...
def build_static_index(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "file_list": files,
        "by_extension": by,
        "html_pages": [f for f in files if f.endswith(".html")],
        "css_files": [f for f in files if f.endswith(".css")],
        "partial_files": [f for f in files if "partial" in f.lower()],
        "recent_commits": _recent_commits(repo_root),
    }
...
def build(repo_root: Path, target_type: str = "static") -> dict[str, Any]:
    return (
        build_node_index(repo_root)
        if target_type == "node"
        else build_static_index(repo_root)
    )
