...
def load_context(affected_paths:list[str], repo_root:Path)->dict[str,str]:
    ...
    for rel in affected_paths:
        ...
        except ValueError as exc: raise PathTraversalError(f'context path {rel!r} resolves outside repo root') from exc
        if not resolved.is_file(): logger.warning('context_builder: file not found: %r', rel); continue
        if resolved.stat().st_size>_MAX_FILE_BYTES: logger.warning('context_builder: skipping oversized file: %r', rel); continue
