...
_ALL_PIPELINES: tuple["PipelineId", ...] = ("seo-aeo-geo", "mobile-ux", "on-brand")
...
def _run_all(cfg: "Settings", r2: "R2Client") -> None:
    ...
    for pid in _ALL_PIPELINES:
        report = asyncio.run(
            RmsPipeline.for_id(pid, cfg, r2, router).run(dry_run=cfg.rms_dry_run)
        )
        logger.info(
            "scheduler: [%s] completed committed=%d futureGuidance=%d",
            pid,
            report.summary.get("committed", 0),
            report.summary.get("futureGuidance", 0),
        )


def build_scheduler(cfg: "Settings", r2: "R2Client") -> Any:
    if BackgroundScheduler is None or CronTrigger is None:
        raise RuntimeError("APScheduler is not installed; scheduler cannot be built")
    sched = BackgroundScheduler(timezone="UTC")
    trig = CronTrigger.from_crontab(cfg.rms_schedule_cron, timezone="UTC")
    sched.add_job(
        _run_all,
        trigger=trig,
